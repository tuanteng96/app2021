...
class ShopDataService {
    ...
    getList(id, pi, ps, tags, keys, stockid, status) {
        return http.get(`/app/index.aspx?cmd=search_prods&key=${keys}&cates=${id}&pi=${pi}&ps=${ps}&stockid=${stockid}&status=${tags === "hot" ? "3" : status}`);
    }
    ...
    getServiceParent(id, stock, pi, ps, ignorepublic, original) {
            return http.get(`/api/v3/app2?get=sv&cid=${id}&stockid=${stock}&takes=Detail,Desc&pi=${pi}&ps=${ps}${ignorepublic ? `&ignorepublic=${ignorepublic}` : ""}${original? `&rootIds=${original}` : ""}`);
    }
